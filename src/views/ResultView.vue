...
<template>
  <div class="result">
    ...
    <p class="result_subtitle">
      ...
      респондентов, чей уровень интеллекта более чем на 15 пунктов отличается от
      среднего в большую или меньшую сторону!
    </p>
    <h1 class="result_alertText">Скорее получите свой результат!</h1>
    <p class="result_copyrights">
    ...
    <p style="font-size: 20px" class="result_timerText">
      {{ minutes }} : {{ seconds }} МИНУТ
    </p>
    <div @click="loadInfo" v-if="time !== 0" class="result_contactInfo">
      <img src="../assets/phone.svg" alt="" />
      <p>Позвонить и прослушать результат</p>
    </div>
    <div v-if="infoFromServer" class="infoFromServer">
      Tel: +00123-{{ infoFromServer?.height }}-{{ infoFromServer?.mass }}
    </div>
  </div>
</template>
